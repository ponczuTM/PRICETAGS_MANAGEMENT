...
def add_device_to_location(device):
    ...
    payload = {
        ...
        "video": "",
        "thumbnail": ""
        # isOnline nada się z backendu na True, ale nie szkodzi zostawić tak jak jest
    }
    try:
        response = requests.post(url, json=payload, headers={"Content-Type": "application/json"}, timeout=10)
        if response.status_code == 201:
...
def main():
    ...
    # 1) Pobierz stan bazy
    db_devices = get_devices_from_database()
    # mapy: clientId -> _id oraz clientId -> ip_z_bazy
    db_clientid_to_id = {d["clientId"]: d["_id"] for d in db_devices}
    db_clientid_to_ip = {d["clientId"]: (d.get("ip") or "") for d in db_devices}
    ...
    # 3) Dodaj nowe urządzenia (nie istniejące w bazie)
    for device in scanned_devices:
        if device["clientid"] not in db_clientid_to_id:
            if add_device_to_location(device):
                # nic więcej — backend ustawi isOnline=True
                pass
    ...
    for device in scanned_devices:
        ...
        new_ip = device["ip"]
        if client_id in db_clientid_to_id:
            device_id = db_clientid_to_id[client_id]
            # jeśli IP w bazie różni się od zeskanowanego — zaktualizuj
            if db_clientid_to_ip.get(client_id, "") != new_ip:
                update_device_ip_in_db(device_id, new_ip)
            # to urządzenie jest dostępne — ustaw online
            set_device_online(device_id)

    # 5) Ustaw OFFLINE tym, których nie było w skanie (wcześniej tu je kasowałeś)
    for client_id, device_id in db_clientid_to_id.items():
        if client_id not in scanned_client_ids:
