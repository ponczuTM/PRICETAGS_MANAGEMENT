...
# Dodawanie urządzenia z isOnline=true
def add_device_to_location(device):
    url = f"{API_BASE}/{LOCATION_ID}/devices/"
    device_data = {
        "clientId": device["clientid"],
...
# Oznaczenie urządzenia jako offline
def mark_device_offline(device_id):
    try:
        response = requests.put(f"{API_BASE}/{LOCATION_ID}/devices/{device_id}/offline")
        if response.status_code == 200:
            print(f"🔻 Oznaczono jako offline: {device_id}")
        else:
            print(f"❌ Błąd ustawiania offline: {response.status_code}")
    except requests.RequestException as e:
        print(f"❌ Błąd PUT offline dla {device_id}: {e}")

# Oznaczenie urządzenia jako online
def mark_device_online(device_id):
    try:
        response = requests.put(f"{API_BASE}/{LOCATION_ID}/devices/{device_id}/online")
        if response.status_code == 200:
            print(f"🟢 Oznaczono jako online: {device_id}")
        else:
            print(f"❌ Błąd ustawiania online: {response.status_code}")
    except requests.RequestException as e:
...
def main():
    ...
    # Krok 3: nowe → dodaj; istniejące → oznacz online
    for device in scanned_devices:
        clientid = device["clientid"]
        if clientid not in db_client_ids:
            add_device_to_location(device)
        else:
            mark_device_online(db_client_ids[clientid])

    # Krok 4: brak w sieci → oznacz offline
    for client_id, device_id in db_client_ids.items():
        if client_id not in scanned_client_ids:
            mark_device_offline(device_id)

    # Krok 5: wypisz aktywne
    print_devices(scanned_devices)
